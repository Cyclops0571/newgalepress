		...
		***********************************************************/
		...
		if (!this.bu_icon) {
			...
			if (this.bu_icon == 'none') {
				this.bu_icon = '/js/zoom_assets/icons.png';
			}
			testOb.remove();
		}		
		...
		this.imgList = [
			...
			{loaded: false, src: imageUrl} // Main image
	...
	Zoomer.prototype = {
		...
		***********************************************************************************************************************/
		updateLocations: function (_x, _y, _sc, loc) {	
			...
			for (var p = 0; p < loc.length; p++) {
				...
				if(loc[p].ob.hasClass("mark"))
				{
					lpx = (loc[p].x);
					lpy = (loc[p].y);
					_sc = 1;
					/*
					var dataPosition = loc[p].ob.attr("data-position").split(',');
					var myX = parseInt(dataPosition[0]);
					if(loc[p].ob.css("left") != "auto")
					{
						myX = parseInt(loc[p].ob.css("left").replace("px",""));
					}
					
					var myY = parseInt(dataPosition[1]);
					if(loc[p].ob.css("top") != "auto")
					{
						myY = parseInt(loc[p].ob.css("top").replace("px", ""));
					}
					
					loc[p].ob.css("left", "auto");
					loc[p].ob.css("top", "auto");
					...
					console.log("myX:" + myX + " myY:" + myY + " lpx:" + lpx + " lpy:" + lpy);
					*/
				}
				...
				*/
				if (lpx !== loc[p].lpx || lpy !== loc[p].lpy && loc[p].vis) {					
					if (prop_transform) {
						...
						//INFO:modified by uet
						loc[p].ob.css(prop_transform, 'translate(' + lpx.toFixed(14) + 'px,' + lpy.toFixed(14) + 'px)' + (loc[p].scale? ' scale(' + (loc[p].ob.hasClass("mark") ? 1 : this._sc) + ')' : ''));
					} else {						
						loc[p].ob.css({
							left: lpx,
							top: lpy						
		...
		***********************************************************************************************************************/
		resize: function (e) {	
			...
			//INFO:Modified by uet
			self.sH = $(window).innerHeight() - $("#pdf-container").offset().top - $("footer").outerHeight();
