SELECT t1.indx, t1.Year, t1.Month, t2.Device AS Device, IFNULL(t2.DownloadCount, 0) AS DownloadCount
FROM (
		SELECT 5 AS indx, YEAR(DATE_ADD('{DATE}', INTERVAL(-1) MONTH)) AS Year, MONTH(DATE_ADD('{DATE}', INTERVAL(-1) MONTH)) AS Month
		UNION ALL
		SELECT 4 AS indx, YEAR(DATE_ADD('{DATE}', INTERVAL(-2) MONTH)) AS Year, MONTH(DATE_ADD('{DATE}', INTERVAL(-2) MONTH)) AS Month
		UNION ALL
		SELECT 3 AS indx, YEAR(DATE_ADD('{DATE}', INTERVAL(-3) MONTH)) AS Year, MONTH(DATE_ADD('{DATE}', INTERVAL(-3) MONTH)) AS Month
		UNION ALL
		SELECT 2 AS indx, YEAR(DATE_ADD('{DATE}', INTERVAL(-4) MONTH)) AS Year, MONTH(DATE_ADD('{DATE}', INTERVAL(-4) MONTH)) AS Month
		UNION ALL
		SELECT 1 AS indx, YEAR(DATE_ADD('{DATE}', INTERVAL(-5) MONTH)) AS Year, MONTH(DATE_ADD('{DATE}', INTERVAL(-5) MONTH)) AS Month
	) t1
	LEFT JOIN (
		SELECT YEAR(`RequestDate`) AS Year, MONTH(`RequestDate`) AS Month, `Device`, COUNT(*) AS DownloadCount
		FROM (
		...
		GROUP BY YEAR(`RequestDate`), MONTH(`RequestDate`), `Device`
