...
<div id="prop-{id}" class="component tooltip hide" componentid="{id}">
    ...
    <div class="upload">
        ...
        <div class="radiogroup">
            <div class="radio {{ ($option == 1 ? 'checked ' : '') }}js-radio" optionvalue="1">{{ __('interactivity.tooltip_icon_default') }}</div>
            <div class="radio {{ ($option == 2 ? 'checked ' : '') }}js-radio" optionvalue="2">{{ __('interactivity.tooltip_icon_new') }}</div>
        ...
        <div class="icon1{{ ($option == 1 ? ' hide' : '') }}">
            ...
            <a class="btn expand uploadfile {{ ($fileSelected == 1 ? ' hide' : '') }}">{{ __('interactivity.tooltip_icon1') }} <i class="icon-upload"></i></a>
        </div>
        ...
        <div class="icon2{{ ($option == 1 ? ' hide' : '') }}">
            ...
            <input type="hidden" name="comp-{id}-filename2" id="comp-{id}-filename2" class="required" value="{{ $filename2 }}" />
            <a class="btn expand uploadfile2 {{ ($fileSelected2 == 1 ? ' hide' : '') }}">{{ __('interactivity.tooltip_icon2') }} <i class="icon-upload"></i></a>
        </div>
    ...
    <div class="settings">
        <div class="component-panel">
            <h5 title="{{ __('interactivity.tooltip_type_tooltip') }}" class="tooltip">{{ __('interactivity.tooltip_type') }} <i class="icon-info-sign"></i></h5>
            ...
            <div class="text">
                ...
                <select class="select js-select" name="comp-{id}-init" id="comp-{id}-init">
                    ...
                    <option value="left"{{ ($init == 'left' ? ' selected="selected"' : '') }}>{{ __('interactivity.tooltip_select1') }}</option>
                    <option value="right"{{ ($init == 'right' ? ' selected="selected"' : '') }}>{{ __('interactivity.tooltip_select2') }}</option>
                    <option value="top"{{ ($init == 'top' ? ' selected="selected"' : '') }}>{{ __('interactivity.tooltip_select3') }}</option>
                    <option value="bottom"{{ ($init == 'bottom' ? ' selected="selected"' : '') }}>{{ __('interactivity.tooltip_select4') }}</option>
                </select>
            </div>
            <!-- end inline -->
