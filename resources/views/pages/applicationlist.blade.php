                        ...
                        <table cellpadding="0" cellspacing="0" width="100%" class="table table-bordered table-striped table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col">{{ HTML::link($route.'?page=1'.((int)Input::get('customerID', 0) > 0 ? '&customerID='.Input::get('customerID', 0) : '').'&search='.$search.'&sort='.$fields[6][2].'&sort_dir='.($sort_dir == 'DESC' ? 'ASC' : 'DESC'), $fields[6][1], ($sort == $fields[6][2] ? ($sort_dir == 'ASC' ? array('class' => 'sort_up') : array('class' => 'sort_down')) : array())) }}</th>
