...
@section('content')
                        ...
                        <div class="input-group">
                          ...
                          <select class="form-control select2" id="ddlApplication"
                            ...
                            @foreach ($applications as $application)
                              <option
                                  value="{{ $application->ApplicationID }}"{{ ($applicationID == (int)$application->ApplicationID ? ' selected="selected"' : '') }}>{{ $application->Name }}</option>
                            @endforeach
                          </select>
                        </div>
                      </div>
                        ...
                        <div class="input-group">
                          ...
                          <select class="form-control select2" id="ddlContent"
                            ...
                            @foreach($contents as $content)
                              <option
                                  value="{{ $content->ContentID }}"{{ ($contentID == (int)$content->ContentID ? ' selected="selected"' : '') }}>{{ $content->Name }}</option>
                            @endforeach
          ...
          <div class="head">
            ...
            <div class="head-panel nm" style="border-top: 1px solid">
              ...
              <div class="hp-info hp-simple pull-left hp-inline">
                ...
                &nbsp;<span class="reportSubtitle">{{ Common::dateRead(Auth::user()->lastLoginDate(), 'H:i') }}</span>
              </div>
            </div>
        ...
        <div class="block bg-light" id="my-info-block">
          <div class="head">
            ...
            <div class="head-panel nm" style="border-top: 1px solid">
              ...
              <div class="hp-info hp-simple pull-left hp-inline">
                <span class="icon-thumbs-up"></span>&nbsp;{{__('common.header_status')}}&nbsp;
                <span class="reportSubtitle"> {{$appDetail->ApplicationStatus()}} </span>
              </div>
            </div>
            <div class="head-panel nm">
  ...
  @php($expiringAppCount = 0)
  @foreach(Auth::user()->expiringApps() as $expApp)
      <?php
      $date1 = new DateTime($expApp->ExpirationDate);
      $diff = $date1->diff(new DateTime());
      ?>
      @if($diff->days<15)
            ...
            <div class="modal-dialog">
              <div class="modal-content">
                ...
                <div class="modal-body clearfix">
                  <div class="modal-body"><?php echo $expApp->getExpireTimeMessage(); ?><span
                        id="dialogText-warning"></span></div>
                </div>
              </div>
            </div>
          </div>
